...
class AngleMomentumStrategy(Strategy):
    """Angle Momentum Strategy."""

    strategy_name: Literal["angle_momentum"] = "angle_momentum"
    ...
    def compute_angle(self, prices: pd.Series) -> float:
        ...
        return math.degrees(np.arctan((prices.iloc[-1] - prices.iloc[0]) / len(prices)))
    ...
    def generate_signal_on_date(
        ...
        for symbol in job.tickers:
            ...
            # else:
            #     print(f"{symbol}: {angle} degrees, rsi: {rsi}, date: {previous_date}")
        ...
        # trade_leg_intents: List[TradeIntentLeg] = []
        # for entry in top_longs + top_shorts:
        #     weight = 1.0 / total_trades if total_trades > 0 else 0
        #     trade_leg_intents.append(
        #         TradeIntentLeg(
        #             symbol=entry["symbol"],
        #             date=entry["date"],
        #             signal=entry["signal"],
        ...
        #     )
        # )
        # return [TradeIntent(date=target_date, legs=trade_leg_intents)]
