...
from components.market import DataSources
...
class SQLiteMarketCache:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS market_data (
                    ...
                    volume REAL,
                    source TEXT,
                    PRIMARY KEY (symbol, datetime, source)
                )
            """
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_symbol_date ON market_data (symbol, datetime, source)"
            )

    def get_cached_data(
        self, symbols: list[str], source: DataSources, start_date: str, end_date: str
    ) -> dict[str, list[dict]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            query = f"""
                ...
                WHERE symbol IN ({placeholders}) AND source = ? AND datetime BETWEEN ? AND ?
                ORDER BY symbol ASC, datetime ASC
            """
            params = [*symbols, source.value, start_date, end_date]
    ...
    def cache_data(self, symbol: str, df: pd.DataFrame, source: DataSources):
        ...
        missing_df["source"] = source.value
...
class SQLiteMetricsStore:
    ...
    def save_portfolio_metrics(self, metrics: PortfolioMetrics, strategy_name: str):
        ...
        # Insert timeseries
        regime_timeseries = (
            metrics.regime_timeseries if metrics.regime_timeseries else {}
        )
